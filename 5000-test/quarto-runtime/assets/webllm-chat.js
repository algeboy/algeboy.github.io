...
const WEBLLM_CONTEXT_MODE_NONE = "none";
const WEBLLM_CONTEXT_MODE_BEFORE_WIDGET = "before-widget";
const WEBLLM_DEFAULT_CONTEXT_CHARS = 3000;
const WEBLLM_MIN_CONTEXT_CHARS = 500;
const WEBLLM_MAX_CONTEXT_CHARS = 12000;
...
const toSingleSpacedText = (text) => trim(String(text ?? "").replace(/\s+/g, " "));

const clamp = (value, min, max) => Math.min(max, Math.max(min, value));

const parseContextChars = (value) => {
  const parsed = Number.parseInt(String(value ?? ""), 10);
  if (!Number.isFinite(parsed)) {
    return WEBLLM_DEFAULT_CONTEXT_CHARS;
  }
  return clamp(parsed, WEBLLM_MIN_CONTEXT_CHARS, WEBLLM_MAX_CONTEXT_CHARS);
};

const normalizeContextMode = (mode) => {
  return mode === WEBLLM_CONTEXT_MODE_BEFORE_WIDGET ? WEBLLM_CONTEXT_MODE_BEFORE_WIDGET : WEBLLM_CONTEXT_MODE_NONE;
};

const getTextBeforeWidget = (widget) => {
  const sectionRoot = widget.closest("section") || document.body;
  if (!sectionRoot || !sectionRoot.contains(widget)) {
    return "";
  }

  const range = document.createRange();
  range.setStart(sectionRoot, 0);
  range.setEndBefore(widget);
  return toSingleSpacedText(range.toString());
};

const getTailByChars = (text, maxChars) => {
  const normalized = toSingleSpacedText(text);
...
const buildInjectedContext = ({ widget, contextMode, contextChars, hiddenContext }) => {
  const parts = [];
  if (contextMode === WEBLLM_CONTEXT_MODE_BEFORE_WIDGET) {
    const priorText = getTailByChars(getTextBeforeWidget(widget), contextChars);
    if (priorText) {
      parts.push(`Page content before this chat widget:\n${priorText}`);
    }
  }

  const hidden = toSingleSpacedText(hiddenContext);
  if (hidden) {
    parts.push(`Author-provided hidden context:\n${hidden}`);
  }
...
const buildMessages = (systemPrompt, injectedContext, conversation, userPrompt) => {
  const messages = [{ role: "system", content: systemPrompt }];
  if (trim(injectedContext)) {
    messages.push({
      role: "system",
      content: `Authoritative chapter excerpt for this turn. Prioritize this context over generic background knowledge. Do not claim the topic is missing if this excerpt addresses it.\n\n${injectedContext}`,
    });
  }
  messages.push(...conversation, { role: "user", content: userPrompt });
  return messages;
};
...
const getEngine = async (sendButton) => {
...
const initWidget = async (widget) => {
  ...
  if (!messagesNode || !promptNode || !sendButton || !resetButton || !clearButton || !modeIndicatorNode || !statusIconNode || !modeToggleButton) {
    return;
  }
  ...
  const contextMode = normalizeContextMode(trim(widget.dataset.webllmContextMode));
  const contextChars = parseContextChars(widget.dataset.webllmContextChars);
  const hiddenContext = trim(widget.dataset.webllmHiddenContext);
  const injectedContext = buildInjectedContext({ widget, contextMode, contextChars, hiddenContext });
  ...
  sendButton.addEventListener("click", async () => {
    ...
    try {
      if (currentMode === WEBLLM_MODE_CACHED) {
        const cachedReply = referenceResponse || "Cached fallback is enabled, but no stored response is configured for this widget.";
      ...
      const engine = await getEngine(sendButton);
      engineReady = true;
      updateModeUi();
      const runCompletion = async (contextForTurn) => {
        const chunks = await engine.chat.completions.create({
          messages: buildMessages(systemPrompt, contextForTurn, conversation, prompt),
          stream: true,
          temperature: 0.2,
          max_tokens: 320,
        });
        ...
        for await (const chunk of chunks) {
          ...
          assistantBubble.textContent = replyText || "...";
        }
      ...
      let reply = "";
      try {
        reply = await runCompletion(injectedContext);
      } catch (firstError) {
        if (trim(injectedContext)) {
          reply = await runCompletion("");
        } else {
          throw firstError;
        }
      }
      ...
      setCookie(WEBLLM_MODE_COOKIE, currentMode);
      updateModeUi();
      assistantBubble.textContent = referenceResponse || "The local model could not be loaded in this browser, and no cached response is configured for this widget.";
